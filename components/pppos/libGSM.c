...
char *getATResponse(char * cmd, int cmdSize, int timeout, int size) {
...
static void pppos_client_task()
{
	...
	imei[15]='\0';
    ESP_LOGI(TAG, "DEVICE IMEI: %s", imei);

	char *iccidResp = getATResponse("AT+ICCID\r\n",10,3000,0);
	iccidResp = iccidResp +9;
	strncpy(iccid,iccidResp,19);
	iccid[19]='\0';
    ESP_LOGI(TAG, "DEVICE ICCID: %s", iccid);
	...
	// Set APN from config
	sprintf(PPP_ApnATReq, "AT+CGDCONT=1,\"IPV4V6\",\"%s\"\r\n", CONFIG_GSM_APN);
